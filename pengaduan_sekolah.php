...
$stmtUser = $conn->prepare("SELECT sekolah_id FROM users WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$error) {
        $stmt = $conn->prepare("INSERT INTO pengaduan (user_id, sekolah_id, kategori_id, judul, deskripsi, foto, status, tanggal_lapor) VALUES (?, ?, ?, ?, ?, ?, 'baru', NOW())");
        $stmt->bind_param("iiisss", $user_id, $sekolah_id, $kategori_id, $judul, $deskripsi, $foto_name);
        if ($stmt->execute()) {
